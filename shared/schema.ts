...
export const storeStock = pgTable("store_stock", {
  ...
  transfersInQty: decimal("transfers_in_qty", { precision: 10, scale: 2 }).default("0.00"),
  transfersOutQty: decimal("transfers_out_qty", { precision: 10, scale: 2 }).default("0.00"),
  interDeptInQty: decimal("inter_dept_in_qty", { precision: 10, scale: 2 }).default("0.00"),
  interDeptOutQty: decimal("inter_dept_out_qty", { precision: 10, scale: 2 }).default("0.00"),
  wasteQty: decimal("waste_qty", { precision: 10, scale: 2 }).default("0.00"),
  writeOffQty: decimal("write_off_qty", { precision: 10, scale: 2 }).default("0.00"),
  adjustmentQty: decimal("adjustment_qty", { precision: 10, scale: 2 }).default("0.00"),
  soldQty: decimal("sold_qty", { precision: 10, scale: 2 }).default("0.00"),
...
// ============================================================
// SRD LEDGER DAILY (New source of truth for SRD inventory)
// One row per day per SRD per item - supports Main and Department stores
// ============================================================

export const SRD_TYPES = ["MAIN", "DEPARTMENT"] as const;
...
export const srdLedgerDaily = pgTable("srd_ledger_daily", {
  ...
  // Opening/Closing
  openingQty: decimal("opening_qty", { precision: 18, scale: 2 }).notNull().default("0.00"),
  closingQty: decimal("closing_qty", { precision: 18, scale: 2 }).notNull().default("0.00"),
  
  // MAIN store movements
  purchaseAddedQty: decimal("purchase_added_qty", { precision: 18, scale: 2 }).notNull().default("0.00"),
  returnsInQty: decimal("returns_in_qty", { precision: 18, scale: 2 }).notNull().default("0.00"), // dept -> main returns received
  reqDepTotalQty: decimal("req_dep_total_qty", { precision: 18, scale: 2 }).notNull().default("0.00"), // main -> dept requisitions issued
  
  // DEPARTMENT store movements
  fromMainQty: decimal("from_main_qty", { precision: 18, scale: 2 }).notNull().default("0.00"), // received from main requisitions
  interInQty: decimal("inter_in_qty", { precision: 18, scale: 2 }).notNull().default("0.00"), // received from other dept SRD
  interOutQty: decimal("inter_out_qty", { precision: 18, scale: 2 }).notNull().default("0.00"), // sent to other dept SRD
  returnsOutToMain: decimal("returns_out_to_main", { precision: 18, scale: 2 }).notNull().default("0.00"), // dept -> main returns sent
  soldQty: decimal("sold_qty", { precision: 18, scale: 2 }).notNull().default("0.00"),
  
  // Shared movements
  wasteQty: decimal("waste_qty", { precision: 18, scale: 2 }).notNull().default("0.00"),
  writeOffQty: decimal("write_off_qty", { precision: 18, scale: 2 }).notNull().default("0.00"),
  adjustmentQty: decimal("adjustment_qty", { precision: 18, scale: 2 }).notNull().default("0.00"), // can be negative
...
// ============================================================
// SRD STOCK MOVEMENTS (Event log for all inventory movements)
// Each movement is tracked for edit/reversal safety
// ============================================================

export const SRD_MOVEMENT_TYPES = [
  "PURCHASE",
  "REQ_MAIN_TO_DEP",
  "RETURN_DEP_TO_MAIN", 
  "TRANSFER_DEP_TO_DEP",
...
export type SrdMovementType = typeof SRD_MOVEMENT_TYPES[number];
...
export const srdStockMovements = pgTable("srd_stock_movements", {
  ...
  fromSrdId: varchar("from_srd_id").references(() => inventoryDepartments.id, { onDelete: "cascade" }),
  toSrdId: varchar("to_srd_id").references(() => inventoryDepartments.id, { onDelete: "cascade" }),
  itemId: varchar("item_id").notNull().references(() => items.id, { onDelete: "cascade" }),

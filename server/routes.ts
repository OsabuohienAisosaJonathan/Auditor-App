...
export async function registerRoutes(
  ...
  app.get("/api/clients/:clientId/department-inventory-links", requireAuth, async (req, res) => {
    try {
      const links = await storage.getDepartmentInventoryLinks(req.params.clientId);
      res.json(links);
    } catch (error: any) {
      res.status(500).json({ error: error.message });
    }
  ...
  app.get("/api/department-inventory-links/:id", requireAuth, async (req, res) => {
    try {
      const link = await storage.getDepartmentInventoryLink(req.params.id);
      if (!link) {
  ...
  app.post("/api/clients/:clientId/department-inventory-links", requireAuth, requireRole(["super_admin", "supervisor"]), async (req, res) => {
    try {
      const { clientDepartmentId, inventoryDepartmentId } = req.body;
      
      if (!clientDepartmentId || !inventoryDepartmentId) {
        return res.status(400).json({ error: "clientDepartmentId and inventoryDepartmentId are required" });
      }
      
      const invDept = await storage.getInventoryDepartment(inventoryDepartmentId);
      if (!invDept) {
        return res.status(404).json({ error: "Inventory department not found" });
      }
      
      const existingByClient = await storage.getDepartmentInventoryLinkByClientDepartment(
        req.params.clientId, 
        clientDepartmentId
      );
      if (existingByClient) {
        return res.status(409).json({ error: "This client department is already linked to an SRD" });
      }
      
      const existingByInventory = await storage.getDepartmentInventoryLinkByInventoryDepartment(
        req.params.clientId, 
        inventoryDepartmentId
      );
      if (existingByInventory) {
        return res.status(409).json({ error: "This SRD is already linked to a client department" });
      }
      
      const link = await storage.createDepartmentInventoryLink({
        clientId: req.params.clientId,
        clientDepartmentId,
        inventoryDepartmentId,
        inventoryType: invDept.inventoryType,
        createdBy: req.session.userId!,
      });
      res.json(link);
    } catch (error: any) {
  ...
  app.put("/api/department-inventory-links/:id", requireAuth, requireRole(["super_admin", "supervisor"]), async (req, res) => {
    try {
      const existing = await storage.getDepartmentInventoryLink(req.params.id);
      if (!existing) {
        return res.status(404).json({ error: "Link not found" });
      }
      
      const { clientDepartmentId, inventoryDepartmentId, status } = req.body;
      
      if (clientDepartmentId && clientDepartmentId !== existing.clientDepartmentId) {
        const existingByClient = await storage.getDepartmentInventoryLinkByClientDepartment(
          existing.clientId, 
          clientDepartmentId
        );
        if (existingByClient && existingByClient.id !== req.params.id) {
          return res.status(409).json({ error: "This client department is already linked to an SRD" });
        }
      }
      
      let inventoryType = existing.inventoryType;
      if (inventoryDepartmentId && inventoryDepartmentId !== existing.inventoryDepartmentId) {
        const existingByInventory = await storage.getDepartmentInventoryLinkByInventoryDepartment(
          existing.clientId, 
          inventoryDepartmentId
        );
        if (existingByInventory && existingByInventory.id !== req.params.id) {
          return res.status(409).json({ error: "This SRD is already linked to a client department" });
        }
        const invDept = await storage.getInventoryDepartment(inventoryDepartmentId);
        if (invDept) {
      ...
      if (clientDepartmentId) updateData.clientDepartmentId = clientDepartmentId;
      if (inventoryDepartmentId) {
        updateData.inventoryDepartmentId = inventoryDepartmentId;
        updateData.inventoryType = inventoryType;
      }
      
      const link = await storage.updateDepartmentInventoryLink(req.params.id, updateData);
      res.json(link);
    } catch (error: any) {
      res.status(400).json({ error: error.message });
    }
  ...
  app.delete("/api/department-inventory-links/:id", requireAuth, requireRole(["super_admin"]), async (req, res) => {
    try {
      await storage.deleteDepartmentInventoryLink(req.params.id);
      res.json({ success: true });
    } catch (error: any) {
      res.status(500).json({ error: error.message });
    }
